...
export const IndicatorPanel: React.FC<IndicatorPanelProps> = ({ indicators, symbol }) => {
  ...
  const rows = [
    ...
    },
    {
      ...
      stateBg: bbConf.bg,
      sub: `↓${formatPrice(indicators.bbLower, symbol)}  ↑${formatPrice(indicators.bbUpper, symbol)}`,
    },
    {
      name: 'ATR (14)',
    ...
    },
    {
      ...
      sub: `+DI ${indicators.diPlus.toFixed(1)}  -DI ${indicators.diMinus.toFixed(1)}`,
    },
  ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 8 }}>
        {rows.map((row) => (
          ...
          >
            ...
            {row.bar !== undefined && (
              <div className="level-bar" style={{ marginBottom: row.sub ? 4 : 0 }}>
                <div
            ...
            {row.sub && (
              <div style={{ fontSize: 10, color: 'var(--t3)', fontFamily: 'JetBrains Mono, monospace', marginTop: 2 }}>{row.sub}</div>
            )}
          </div>
        ))}
      </div>
